...
export default function AdminAnimatedGrid({ products, onFullEdit }: { products: Product[]; onFullEdit?: (p: Product) => void }) {
  ...
  const handleQuickEdit = async (productId: string, formData: FormData) => {
    setSaving(productId);
    try {
      ...
      {products.map(p => (
        <motion.div key={p.id} className="card" variants={item} whileHover={{ y:-6 }}>
          ...
          <details style={{ marginTop:'.6rem' }}>
            ...
            <form action={(fd) => handleQuickEdit(p.id, fd)} style={{ display:'grid', gap:'.4rem', marginTop:'.5rem' }}>
              ...
              <button type="submit" disabled={saving === p.id} style={{ fontSize:'.7rem', padding:'.4rem .6rem', opacity: saving === p.id ? 0.6 : 1 }}>
